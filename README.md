...
- Anti-hacks security: in case where the user's _refreshToken_ is not inside the database -> it means the _refreshToken_ was used by someone else (I suspect it's a hacker) and I alert the user about the potential threat.
  - However, in my *multi-device* logic the issue of "*missing refreshToken*" could simply mean that a trusted family user - logged on another device - have just used the option "*logout all devices*" so I had to modify the alert to include more empathy and remind my users of such case scenario where they have to communicate it out with them and be assured whether they were hacked or not.
